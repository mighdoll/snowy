...
case class LoadedGeometries(threeSleds: ThreeSleds, threeSnowballs: ThreeSnowballs)
...
class GeometryLoader {
  ...
  private val geometryFutures: Seq[Future[Geometry]] =
    loader.loadGeometries("sled/body.json", "sled/skis.json", "snowball/snowball.json")
  ...
  val threeGroupsFuture: Future[LoadedGeometries] = geometriesLoaded.map {
    case Seq(sled, skis, snowball) =>
      LoadedGeometries(new ThreeSleds(sled, skis), new ThreeSnowballs(snowball))
  }
}
