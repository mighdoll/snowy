...
object ClientDraw {
  ...
  def drawState(snowballs: Store[Snowball],
    ...
    ctx.beginPath()
    ctx.moveTo(mySled.radius * portal.scale / 7, mySled.radius * portal.scale * 10 / 7)
    ctx.lineTo(-mySled.radius * portal.scale / 7, mySled.radius * portal.scale * 10 / 7)
    ctx.lineTo(0, mySled.radius * portal.scale * 12 / 7)
    ctx.closePath()
    ctx.fill()
